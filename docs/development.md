...
- Keep effects responsible for setup and cleanup: timers, subscriptions, and async responses must stop affecting state after teardown or a connection/query change. Preserve query cancellation, connection cache isolation, and pagination generation checks when changing asynchronous flows.
